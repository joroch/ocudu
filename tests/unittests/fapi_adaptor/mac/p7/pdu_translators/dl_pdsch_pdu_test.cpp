...
static void validate_pdsch_information(const pdsch_information& pdsch_cfg, const fapi::dl_pdsch_pdu& fapi_pdu)
{
  ...
  const auto* ra_type_1 = std::get_if<fapi::resource_allocation_type_1>(&fapi_pdu.resource_alloc);
  ASSERT_TRUE(ra_type_1);
  ASSERT_EQ(prb_cfg.type1().start(), ra_type_1->rb_start);
  ASSERT_EQ(prb_cfg.type1().length(), ra_type_1->rb_size);
}
...
TEST(mac_fapi_pdsch_pdu_conversor_test, valid_sib1_pdu_should_pass)
{
  ...
  // CSI-RS rm.
  span<const uint16_t> csi_rm = fapi_pdu.csi_for_rm;
