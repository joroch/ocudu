...
/// approximate channel representations, without per-subcarrier coefficients.
...
public:
  ...
  /// \param[in] tolerance Maximum distance between normalized matrices, in the range of [0, 2]. A value of 0 means both
  /// matrices are identical (A = B), whereas a value of 2 means they point to opposite directions (B = -A).
  ...
  channel_matrix operator-(const channel_matrix& other) const
  {
    ...
    span<const cf_t> raw_data    = this->data.get_data();
