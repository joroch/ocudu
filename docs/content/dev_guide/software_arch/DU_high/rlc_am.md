...
In [Fig. 6](#rlctxentity), we can see a simplified illustration of the RLC AM Tx entity.
There, we can see two threads do the work of pushing SDUs into the RLC and pulling PDUs from the RLC.

<a id="rlctxentity"></a>
![image](dev_guide/source/software_arch/source/DU_high/source/.imgs/rlc_tx_entity.png)
...
This will be kept until the transmitted PDUs are ACKed in sequence. This is illustrated in [Fig. 7](#rlctxwindow).
...
<a id="rlctxwindow"></a>
![image](dev_guide/source/software_arch/source/DU_high/source/.imgs/rlc_tx_window.png)
...
This is illustrated in [Fig. 9](#rlcrxwindow), where we can see four state variables: *RX_Next*, *RX_Highest_Status*, *RX_Next_Status_Trigger* and *RX_Next_Highest*.
...
<a id="rlcrxwindow"></a>
![image](dev_guide/source/software_arch/source/DU_high/source/.imgs/rlc_rx_window.png)
...
An illustration of the process of generating the Status Report can be found in [Fig. 10](#rlcstatusgeneration).

<a id="rlcstatusgeneration"></a>
![image](dev_guide/source/software_arch/source/DU_high/source/.imgs/rlc_status_report_transmission.png)

When the RLC receives a Status Report, it must be passed to the TX entity for processing. The TX entity will use the received status report to update the TX window and the RETX queue. Because both the *UE executor* thread and *Cell Executor* thread can update both the TX window and RETX queue, both of these variables need to be protected with a lock. An illustration of the process of handling the Status Report can be found in [Fig. 11](#rlcstatushandling).

<a id="rlcstatushandling"></a>
![image](dev_guide/source/software_arch/source/DU_high/source/.imgs/rlc_status_report_handling.png)
