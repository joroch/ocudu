...
#include "test_doubles/nrppa/nrppa_test_message_validators.h"
#include "test_doubles/nrppa/nrppa_test_messages.h"
...
#include "unittests/cu_cp/cu_cp_test_environment.h"
...
static std::vector<std::vector<supported_tracking_area>> make_amf_test_config()
{
  ...
  plmn_item               plmn         = plmn_item{plmn_identity::test_value(), std::vector<s_nssai_t>{s_nssai}};
  tac_t                   tac          = 7;
...
public:
  ...
  asn1::nrppa::nr_ppa_pdu_c get_nrppa_pdu(ngap_message ngap_pdu_)
  {
  ...
  ngap_message generate_valid_dl_ue_associated_nrppa_transport_message(const ue_context* ue_ctxt, byte_buffer nrppa_pdu)
  {
  ...
  ngap_message generate_valid_dl_non_ue_associated_nrppa_transport_message(byte_buffer nrppa_pdu)
  {
  ...
  [[nodiscard]] bool send_f1ap_trp_information_response(unsigned du_id, std::vector<trp_id_t> trp_ids)
  {
    report_fatal_error_if_not(not this->get_amf().try_pop_rx_pdu(ngap_pdu),
                              "there are still NGAP messages to pop from AMF");
    report_fatal_error_if_not(not this->get_du(du_idx).try_pop_dl_pdu(f1ap_pdu),
  ...
  [[nodiscard]] bool run_successful_trp_information_procedure()
  {
    ...
    if (not await_nrppa_trp_information_response(9)) {
      return false;
    }
