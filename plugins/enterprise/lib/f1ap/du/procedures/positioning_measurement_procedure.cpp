...
{
public:
  positioning_measurement_procedure(const positioning_meas_request_s& msg_,
...
private:
  ...
  const positioning_meas_request_s msg;
...
bool positioning_measurement_procedure::validate_request() const
{
...
async_task<du_positioning_meas_response> positioning_measurement_procedure::request_positioning_measurement()
{
  ...
  if (msg->srs_configuration_present) {
    du_req.srs_carriers = asn1_helper::from_asn1(msg->srs_configuration);
  }

  return du_mng.request_positioning_measurement(du_req);
}

void positioning_measurement_procedure::send_response() const
{
...
void positioning_measurement_procedure::send_failure() const
{
  f1ap_message resp_msg;
...
async_task<void> odu::start_positioning_measurement_procedure(const positioning_meas_request_s& msg,
                                                              f1ap_du_configurator&             du_mng,
                                                              f1ap_message_notifier&            cu_notifier)
{
  return launch_async<positioning_measurement_procedure>(msg, du_mng.get_positioning_handler(), cu_notifier);
}
