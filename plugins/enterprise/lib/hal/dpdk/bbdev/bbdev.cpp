...
expected<::rte_bbdev_info> dpdk::bbdev_start(const bbdev_acc_configuration& cfg, ocudulog::basic_logger& logger)
{
  ...
  // Enable interrupts. Not possible when using 'vfio-pci' if OFH is using DPDK as well.
  ...
  if (err < 0) {
    ...
    logger.warning("[bbdev] failed to enable interrupts for device {}: '{}'.{}", cfg.id, strerror(err), bbdev_vf_error);
  }
