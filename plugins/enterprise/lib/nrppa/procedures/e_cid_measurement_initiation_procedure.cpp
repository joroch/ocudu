...
void e_cid_measurement_initiation_procedure::send_ul_nrppa_pdu(const asn1::nrppa::nr_ppa_pdu_c& e_cid_meas_init_outcome)
{
  // Pack into PDU.
  ul_nrppa_pdu =
      pack_into_pdu(e_cid_meas_init_outcome,
                    e_cid_meas_init_outcome.type().value == asn1::nrppa::nr_ppa_pdu_c::types_opts::successful_outcome
                        ? "ECIDMeasInitiationResponse"
                        : "ECIDMeasInitiationFailure");

  // Log Tx message.
  log_nrppa_message(ocudulog::fetch_basic_logger("NRPPA"), Tx, ul_nrppa_pdu, e_cid_meas_init_outcome);
