...
void ocudu::fapi::log_error_indication(const error_indication_message& msg,
                                       unsigned                        sector_id,
...
void ocudu::fapi::log_crc_indication(const crc_indication_message& msg,
                                     unsigned                      sector_id,
                                     ocudulog::basic_logger&       logger)
{
  fmt::memory_buffer buffer;
...
static void log_csi_rs_pdu(const dl_csi_rs_pdu& pdu, fmt::memory_buffer& buffer)
{
  if (pdu.type == csi_rs_type::CSI_RS_NZP) {
    fmt::format_to(std::back_inserter(buffer),
                   "\n\t- NZP-CSI-RS crbs={}:{} row={} symbL0={} symbL1={}",
                   pdu.start_rb,
                   pdu.num_rbs,
                   pdu.row,
                   pdu.symb_L0,
  ...
  if (pdu.type == csi_rs_type::CSI_RS_ZP) {
    fmt::format_to(std::back_inserter(buffer), "\n\t- ZP-CSI-RS crbs={}:{} row={}", pdu.start_rb, pdu.num_rbs, pdu.row);
    return;
  }
}
...
void ocudu::fapi::log_dl_tti_request(const dl_tti_request_message& msg,
                                     unsigned                      sector_id,
                                     ocudulog::basic_logger&       logger)
{
  fmt::memory_buffer buffer;
...
void ocudu::fapi::log_rach_indication(const rach_indication_message& msg,
                                      unsigned                       sector_id,
                                      ocudulog::basic_logger&        logger)
{
  fmt::memory_buffer buffer;
...
void ocudu::fapi::log_rx_data_indication(const rx_data_indication_message& msg,
                                         unsigned                          sector_id,
                                         ocudulog::basic_logger&           logger)
{
  fmt::memory_buffer buffer;
...
void ocudu::fapi::log_tx_data_request(const tx_data_request_message& msg,
                                      unsigned                       sector_id,
                                      ocudulog::basic_logger&        logger)
{
...
static void log_uci_pucch_f0_f1_pdu(const uci_pucch_pdu_format_0_1& pdu, fmt::memory_buffer& buffer)
{
  ...
  if (pdu.timing_advance_offset != std::numeric_limits<decltype(pdu.timing_advance_offset)>::min()) {
    fmt::format_to(std::back_inserter(buffer), " ta_ns={}", pdu.timing_advance_offset.to_seconds() * 1e9);
  }
  if (pdu.rsrp != std::numeric_limits<decltype(pdu.rsrp)>::max()) {
...
static void log_uci_pucch_f234_pdu(const uci_pucch_pdu_format_2_3_4& pdu, fmt::memory_buffer& buffer)
{
  ...
  if (pdu.timing_advance_offset != std::numeric_limits<decltype(pdu.timing_advance_offset)>::min()) {
    fmt::format_to(std::back_inserter(buffer), " ta_ns={}", pdu.timing_advance_offset.to_seconds() * 1e-9);
  }
  if (pdu.rsrp != std::numeric_limits<decltype(pdu.rsrp)>::max()) {
    fmt::format_to(std::back_inserter(buffer), " rsrp={:.1f}", to_uci_ul_rsrp(pdu.rsrp));
  }
...
void ocudu::fapi::log_uci_indication(const uci_indication_message& msg,
                                     unsigned                      sector_id,
                                     ocudulog::basic_logger&       logger)
{
  fmt::memory_buffer buffer;
...
void ocudu::fapi::log_srs_indication(const srs_indication_message& msg,
                                     unsigned                      sector_id,
                                     ocudulog::basic_logger&       logger)
{
  fmt::memory_buffer buffer;
...
void ocudu::fapi::log_ul_tti_request(const ul_tti_request_message& msg,
                                     unsigned                      sector_id,
                                     ocudulog::basic_logger&       logger)
{
  fmt::memory_buffer buffer;
...
void ocudu::fapi::log_slot_indication(const slot_indication_message& msg,
                                      unsigned                       sector_id,
                                      ocudulog::basic_logger&        logger)
{
...
void ocudu::fapi::log_ul_dci_request(const ul_dci_request_message& msg,
                                     unsigned                      sector_id,
                                     ocudulog::basic_logger&       logger)
{
  fmt::memory_buffer buffer;
