...
{
  if (ue_cfg.init_bwp().cfg.ul.ul_ded() == nullptr or not ue_cfg.init_bwp().cfg.ul.ul_ded()->srs_cfg.has_value()) {
    return;
  }
...
{
  if (new_ue_cfg.init_bwp().cfg.ul.ul_ded() != nullptr and old_ue_cfg.init_bwp().cfg.ul.ul_ded() != nullptr and
      new_ue_cfg.init_bwp().cfg.ul.ul_ded()->srs_cfg.has_value() and
      old_ue_cfg.init_bwp().cfg.ul.ul_ded()->srs_cfg.has_value()) {
    // Both old and new UE config have SRS config.
    const auto& new_srs_cfg = new_ue_cfg.init_bwp().cfg.ul.ul_ded()->srs_cfg.value();
    const auto& old_srs_cfg = old_ue_cfg.init_bwp().cfg.ul.ul_ded()->srs_cfg.value();
    ...
    const positioning_measurement_request::cell_info& cell_req)
{
  ...
  if (cell_req.ue_index.has_value()) {
    ...
    const auto* ul_cfg = u.get_pcell().cfg().init_bwp().cfg.ul.ul_ded();
                                                  ...
                                                  const periodic_srs_info&      srs_opportunity) const
{
  ...
  if (is_crnti(srs_opportunity.rnti)) {
    ...
    srs_res_list = ue_cfg->init_bwp().cfg.ul.ul_ded()->srs_cfg.value().srs_res_list;
