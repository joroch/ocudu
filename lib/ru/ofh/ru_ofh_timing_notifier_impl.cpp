...
void ru_ofh_timing_notifier_impl::on_new_symbol(const ofh::slot_symbol_point_context& symbol_point_context)
{
  if (symbol_point_context.symbol_point.get_symbol_index() == half_slot_symbol) {
    timing_notifier.on_ul_half_slot_boundary(symbol_point_context.symbol_point.get_slot_symbol_point().get_slot());
  }

  if (symbol_point_context.symbol_point.get_symbol_index() == full_slot_symbol) {
    timing_notifier.on_ul_full_slot_boundary(symbol_point_context.symbol_point.get_slot_symbol_point().get_slot());
  }

  // New slots start on symbol index 0.
  if (symbol_point_context.symbol_point.get_symbol_index() == 0) {
    timing_notifier.on_tti_boundary(
        tti_boundary_context{.slot = symbol_point_context.symbol_point.get_slot_extended() + nof_slot_offset_du_ru,
