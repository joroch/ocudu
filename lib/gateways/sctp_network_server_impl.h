...
/// deferred to the ctrl_exec thread.
...
private:
  ...
  /// Handle SCTP COMM UP event.  Defers pending_connect success signaling to ctrl_exec.
  void
  handle_sctp_comm_up(const struct sctp_assoc_change& assoc_change, const sockaddr& src_addr, socklen_t src_addr_len);

  /// Handle SCTP CANT_STR_ASSOC event. Defers pending_connect failure signaling to ctrl_exec.
  ...
  std::map<transport_layer_address, manual_event<bool>> pending_connects; // handled by ctrl_exec
};
