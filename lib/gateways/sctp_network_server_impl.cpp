...
async_task<bool> sctp_network_server_impl::connect(transport_layer_address dest_addr)
{
  return launch_async([this, dest_addr, event_ptr = (manual_event<bool>*)nullptr, result = false](
...

    {
      ...
      int                                  ret = ::sctp_connectx(get_socket_fd(), native_addr.addr, 1, &assoc_id);
                                                   ...
                                                   socklen_t                       src_addr_len)
{
  ...
  // If this was a pending outgoing connection, defer signaling success to ctrl_exec.
  transport_layer_address peer_addr = assoc_ctxt.addr;
  while (not ctrl_exec.defer([this, peer_addr]() {
    ...
    logger.error("{}: Failed to defer pending connect resolution to ctrl_exec. Cause: Task queue is full. Retrying...",
                 node_cfg.if_name);
                                                               ...
                                                               socklen_t       src_addr_len)
{
  ...
  // Defer pending connect failure signaling to ctrl_exec.
  while (not ctrl_exec.defer([this, addr]() {
                                                                      ...
                                                                      sctp_network_association_factory& assoc_factory_)
{
  ...
  std::unique_ptr<sctp_network_server_impl> server{
      new sctp_network_server_impl(sctp_cfg, broker_, io_rx_executor_, ctrl_exec_, assoc_factory_)};
