...
void sctp_network_server_impl::receive()
{
  ...
  if (rx_bytes == -1) {
    if (errno != EAGAIN) {
      ...
      // Stop handling new SCTP events from io_broker, defer association cleanup to ctrl_exec.
      io_sub.reset();
      while (not ctrl_exec.defer([this]() { handle_socket_shutdown(nullptr); })) {
  ...
  // Defer all processing after sctp_recvmsg to ctrl_exec.
  auto payload = std::vector<uint8_t>(temp_recv_buffer.begin(), temp_recv_buffer.begin() + rx_bytes);
  while (not ctrl_exec.defer([this, payload = std::move(payload), msg_flags, sri, msg_src_addr, msg_src_addrlen]() {
