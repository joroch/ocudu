...
void xnap_impl::handle_cho_cancel_required(ue_index_t ue_index, const nr_cell_global_id_t& target_cgi)
{
  // Look up the candidate's peer_xnap_ue_id from the UE context.
  if (!ue_ctxt_list.contains(ue_index)) {
...
void xnap_impl::handle_handover_success_required(ue_index_t ue_index, const nr_cell_global_id_t& cgi)
{
  if (!ue_ctxt_list.contains(ue_index)) {
...
void xnap_impl::handle_handover_success(const asn1::xnap::ho_success_s& msg)
{
  ...
  ue_index_t ue_index = ue_ctxt_list[local_xnap_ue_id].ue_ids.ue_index;
...
void xnap_impl::handle_conditional_ho_cancel(const asn1::xnap::conditional_ho_cancel_s& msg)
{
  ...
  ue_index_t ue_index = ue_ctxt_list[local_id].ue_ids.ue_index;
