...
/// Read the first line from a sysfs file. Returns an empty string on failure.
...
/// Parse a floating-point scale value from sysfs. Returns false on failure.
...
/// Parse "event=0xNN" string and extract the hex value into config. Returns false on failure.
...
/// Open a perf RAPL fd for the given PMU type and event config on CPU 0. Returns -1 on failure.
...
perf_event_powercap_reader::perf_event_powercap_reader(unique_fd pkg_fd_,
                                                       ...
                                                       double    core_scale_,
                                                       bool      has_core_) :
  pkg_fd(std::move(pkg_fd_)),
  pkg_scale(pkg_scale_),
  core_fd(std::move(core_fd_)),
  core_scale(core_scale_),
  has_core(has_core_)
{
}
...
energy_consumption perf_event_powercap_reader::read_consumed_energy() const
{
  ...
  probe.cpu_core_consumed_uj    = has_core ? read_raw_uj(core_fd, core_scale) : 0;
  return probe;
}

std::unique_ptr<energy_consumption_reader>
resource_usage_utils::build_perf_event_reader(ocudulog::basic_logger& logger)
{
  // Read RAPL PMU type.
  ...
  double core_scale = 0.0;
  {
    ...
    if (parse_event_config(read_sysfs_line(powercap_core_event_path), core_event_config)) {
      if (parse_scale(read_sysfs_line(powercap_core_scale_path), core_scale)) {
        ...
        has_core = core_fd.is_open();
      }
    }
  }

  return std::make_unique<perf_event_powercap_reader>(
      std::move(pkg_fd), pkg_scale, std::move(core_fd), core_scale, has_core);
}
