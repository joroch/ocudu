...
public:
  ...
  span<ci16_t> write_port(unsigned i_port, baseband_gateway_timestamp in_timestamp)
  {
    ...
    ocudu_assert(empty_ports[i_port], "Attempting to write to non-empty buffer.");

    empty_ports[i_port] = false;
  ...
  bool is_empty() const
  {
    return std::all_of(empty_ports.begin(), empty_ports.end(), [](bool port_empty) { return port_empty; });
  }
