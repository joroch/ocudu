...
/// Usage: release <serving_pci> <rnti> [<target_arfcn> [scs <15|30|60|120>]]
...
public:
  ...
  std::string_view get_description() const override
  {
    return " <serving_pci> <rnti> [<target_arfcn> [scs <15|30|60|120>]]: release UE to RRC_IDLE, optionally with NR "
           "redirection";
  }
  ...
  void execute(span<const std::string> args) override
  {
    if (args.size() < 2) {
      fmt::print(
          "Invalid release command. Usage: release <serving_pci> <rnti> [<target_arfcn> [scs <15|30|60|120>]]\n");
      return;
    }
    ...
    if (arg != args.end()) {
      ...
      if (arg != args.end()) {
        if (*arg == "scs") {
          ...
          switch (scs_khz.value()) {
            case 15:
              ssb_scs = subcarrier_spacing::kHz15;
              break;
            case 30:
              ssb_scs = subcarrier_spacing::kHz30;
              break;
            case 60:
              ssb_scs = subcarrier_spacing::kHz60;
              break;
            case 120:
              ssb_scs = subcarrier_spacing::kHz120;
              break;
            default:
              fmt::print("Invalid SCS value {}. Supported: 15, 30, 60, 120.\n", scs_khz.value());
              return;
          }
        } else {
          fmt::print(
              "Unknown argument '{}'. Usage: release <serving_pci> <rnti> [<target_arfcn> [scs <15|30|60|120>]]\n",
              *arg);
          return;
        }
      }
    ...
    cu_cp.get_mobility_command_handler().trigger_release(
